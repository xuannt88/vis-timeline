...
export default class ClusterGenerator {
    ...
    */
     ...
     * @param {{maxItems: number, clusterCriteria: function, titleTemplate: string, contentTemplate: string}} options             
    ...
    */
    ...
    _getClusterForItems(clusterItems, group, oldClusters, options) {
        ...
        let contentTemplate = options.contentTemplate || '';
        const conversion = {
        ...
        const content = contentTemplate.replace(/{count}/, clusterItems.length);
        const clusterContent = '<div title="' + title + '">' + content + '</div>';
